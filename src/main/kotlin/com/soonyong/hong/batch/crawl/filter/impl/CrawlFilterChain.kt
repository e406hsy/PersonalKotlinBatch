...
class CrawlFilterChain(
    ...
    enum class DelegateCondition(
        private val predicateGenerator: BiFunction<Predicate<Element>, Predicate<Element>, Predicate<Element>>
    ) {
        ...
        fun getPredicate(delegate: CrawlFilter, next: CrawlFilter): Predicate<Element> {
            return predicateGenerator.apply(Predicate { input: Element ->
                delegate.isAllowed(
                    input
                )
            }, Predicate { input: Element ->
                next.isAllowed(
                    input
                )
            })
